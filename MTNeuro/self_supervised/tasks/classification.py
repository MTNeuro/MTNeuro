...
def compute_accuracy(encoder, classifier, loader, transform=None, device='cpu', num_classes=4):
...
def generate_plots(acc, acc_class, plot_path, encoder, classifier, loader, transform=None, device='cpu'):
    ...
    for lab in range(num_categories):
        ...
        ax.scatter(pca_proj[indices,0],pca_proj[indices,1], c=np.array(cmap(lab)).reshape(1,4), label = lab ,alpha=0.8)
    ax.legend(fontsize='large', markerscale=2)
    fig.savefig(plot_path + 'rep.png')
...
def train_classifier(encoder, classifier, train_loader, test_loader, optimizer, num_epochs, scheduler=None,
                     ...
                     tqdm_progress=False, smoothing_factor=0.5, plot_path = None, criterion = None, num_classes = 4):
    ...
    for epoch in tqdm(range(num_epochs), disable=not tqdm_progress):
        ...
        acc_value, acc_class_value, conf = compute_accuracy(encoder, classifier, test_loader, transform=test_transform, device=device, num_classes=num_classes)
    ...
    if plot_path is not None:
        generate_plots(acc_value, acc_class_value, plot_path, encoder, classifier, test_loader, transform=test_transform, device=device)
